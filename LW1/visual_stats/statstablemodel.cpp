...
void StatsTableModel::setIsNotSaved()
{
    m_isSaved = false;
}
...
void StatsTableModel::sort(int column, Qt::SortOrder order)
{
    ...
    std::function<bool(std::pair<QString, int> const &item1, std::pair<QString, int> const &item2)> fLess;
    switch (column)
    {
    case COLUMN_ID_NAME:
        if (order == Qt::AscendingOrder)
        {
            fLess = [](std::pair<QString, int> const &item1, std::pair<QString, int> const &item2) {
                return QString::compare(item1.first, item2.first, Qt::CaseInsensitive) < 0;
            };
        }
        else
        {
    ...
    qSort(m_statsModel.begin(), m_statsModel.end(), fLess);
    emit layoutChanged();
}
